...
export class Migration20220704192235 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      'update "stat" set "user_id" = (select "id" from "user" where "telegram_id" = "stat"."user_telegram_id");',
    )
    this.addSql('alter table "stat" alter column "user_id" set not null;')
    this.addSql(
      'alter table "stat" add constraint "stat_user_id_foreign" foreign key ("user_id") references "user" ("id") on update cascade on delete cascade;',
    )
  ...
  async down(): Promise<void> {
    ...
    this.addSql(
      'alter table "stat" add constraint "stat_user_telegram_id_foreign" foreign key ("user_telegram_id") references "user" ("telegram_id") on update cascade on delete cascade;',
    )
